...
导入图片（通过拖拽、粘贴或工具栏选择）支持两种处理模式：

- **图床上传（默认）**：图片自动上传至配置的图床存储，并在当前光标处插入 Markdown 图片语法 `![文件名](url)`。
- **端侧 OCR 识别**：在编辑器设置（或命令面板）中开启“导入图片时启用 OCR”后，图片不再上传至远端，而是通过浏览器端 PaddleOCR.js 引擎在独立 Web Worker 中提取纯文本并直接插入光标位置。
- **增强模型切换**：默认使用轻量模型（PP-OCRv6 tiny，加载迅速、内存占用低）；开启“使用增强 OCR（约 30 MB）”后自动切换至更高精度的 PP-OCRv6 small 模型。
- **按需加载与并发保护**：OCR 依赖与模型仅在功能启用且首次导入图片时按需加载；多图识别由内部互斥锁串行排队执行，兼顾性能与内存稳定性。
...
- **本地 OCR 分流保护**：若在编辑器设置中启用了“导入图片时启用 OCR”，图片将直接转入端侧文字识别流程，不触发远端上传。
